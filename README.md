...
- **Plugin System**: Support plugin system, allowing for easy extension of functionality and integration of custom features like CoT with reflection, multi-step reasoninig. Support for setting plugin-specific parameters, enabling fine-tuned control over plugin behavior and outputs.
...
Plugins placed in `plugins/` directory, read [plugins/README.md](plugins/README.md) for more details.

### Caches

After installing new models via Ollama, you need to click "Clear cache" button on right top to clear the cache. The cache will be auto expired in one day.
